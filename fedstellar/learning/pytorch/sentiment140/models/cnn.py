...
class SentimentModelCNN(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        self.convs = torch.nn.ModuleList([
            torch.nn.Conv2d(in_channels=1, out_channels=self.n_filters, kernel_size=(fs, 300)) for fs in self.filter_sizes
        ])
        self.fc = torch.nn.Linear(len(self.filter_sizes) * self.n_filters, self.output_dim)
